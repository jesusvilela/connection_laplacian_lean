...
/-! ### Linear-algebra helpers (kernel dim under similarity / reindex / block)

Three pure-LinAlg lemmas over an arbitrary field, used by
`laplacian_kernel_dim_decomposes` and `scalarLap_cover_kernel_dim`
to lift a similarity + reindex + `fromBlocks`-diagonal matrix identity
into a kernel-dim sum. -/

/-- Similarity by an invertible matrix preserves `finrank (ker (toLin' ·))`. -/
lemma finrank_ker_toLin'_conj
  ...
  let e : (n → K) ≃ₗ[K] (n → K) :=
    LinearEquiv.ofLinear (Matrix.toLin' P) (Matrix.toLin' P⁻¹)
      (by rw [← Matrix.toLin'_mul, hmul₁, Matrix.toLin'_one])
      (by rw [← Matrix.toLin'_mul, hmul₂, Matrix.toLin'_one])
  have hfact :
      Matrix.toLin' (P * L * P⁻¹) =
        ((e : (n → K) →ₗ[K] (n → K))).comp
  ...
  rw [hfact, LinearEquiv.ker_comp e, LinearMap.ker_comp]
...
/-- Reindexing via an equivalence on both sides preserves
`finrank (ker (toLin' ·))`. -/
...
/-- For a block-diagonal `fromBlocks A 0 0 B`, kernel finrank splits as a sum. -/
lemma finrank_ker_toLin'_fromBlocks_diag
  ...
  have hker_iff :
    ...
    have hT_mulvec :
      ...
      ext i
      cases i with
      | inl i => simp [Matrix.zero_mulVec, Function.comp]
      | inr j => simp [Matrix.zero_mulVec, Function.comp]
    rw [hT_mulvec]
    constructor
    · intro hx
      refine ⟨?_, ?_⟩
      · funext i; have := congrFun hx (Sum.inl i); simpa using this
      · funext j; have := congrFun hx (Sum.inr j); simpa using this
    · rintro ⟨hA, hB⟩
      ext i
      cases i with
      | inl i => have := congrFun hA i; simpa using this
      | inr j => have := congrFun hB j; simpa using this
  let kerEq :
